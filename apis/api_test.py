    ...
    while True:
        try:
            selected_index = input(
                'Please enter an index to get details (or \'quit\' to quit):')
            if selected_index == 'quit':
                break
            print (
                '---------- {0} ----------'.format(restaurant_list[int(selected_index) - 1]['name']))

            # query again to get the details
            details = get_details(
                restaurant_list[int(selected_index) - 1]['place_id'])
            ...
            if ("opening_hours" in details):
                ...
                if ('price_level' in details):
                    ...
                    print("No Price Level Given")
            else:
                print ("No opening hours offered")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Get all restaurants near a location from Google Maps')
